...
int testCubic()
{
    // Test data
    int nTests = 13;
    // Given tests in W. Kahan's papers
    double tests[][4] = {
        {0, 0, 0, 0},
        {-1, 0, 0, -1},
        {1, 3, 3, 1},
...
int testQuartic()
{
    ...
    int nTests = 18;
    double tests[][5] = {
        {1.0,-2.63453369092816, 11.47594580628995,-3.01193978276912,0.17801932206896}, /* 0 */
        {1.0,-2.63453369092816,2.47594580628995,-0.969679092568218,0.133302415104222}, /* 1 */
        {16.0,-16.62910939950919,8.30968497920564,-1.86739743049595,0.12844638447658}, /* 2 */
        {1.0, 6, -5, -10, -3},  /* 3 */
        {1.0, -2.35220164231774431, 2.00357193581024839, -0.732009944503505094, 0.0963567270541595411}, /* 4 */
        {4213952.415, -6257401.32508918, 2907561.77975298, -488990.675178038, 20927.9939160697}, /* 5 */
        {57341.571, -161727.801652109, 168247.656267706, -76482.4511413003, 12801.3771995616}, /* 6 */
        {1.624401562e-21, -4.54214506896612e-21, 4.72995343184905e-21, -2.17240456536774e-21, 3.70944181855249e-22}, /* 7 */
        {1.324862655e-21, -2.25090952994387e-21, 1.2704566968795e-21, -2.55885000154948e-22, 9.1991026101414e-24}, /* 8 */
        {3852.061343, -8484.23316822249, 6538.97688624379, -2056.39000615966, 212.050414780893}, /* 9 */
        {3340.933089, -4990.31721011812, 3726.99260868093, -1391.74258071197, 194.890453343193}, /* 10 */
        {3294.612452, -5742.33193538764, 3753.21565167956, -1090.27543254508, 118.768207298396}, /* 11 */
        {1.63064755140304e-16, -2034.51169316325, 1215.54655528735, -322.775442718126, 32.1410930237635}, /* 12 */
        {1606.268505, -3167.86593395418, 2156.86431456392, -586.683298630047, 53.9570421132115}, /* 13 */
        {3063.145275, -4196.79839872473, 2875.00513659746, -290.139062231677, 5.32413681477471}, /* 14 */
        {1836.085081, -3157.51102285065, 2036.23649359839, -378.014005550609, 27.7813065153978}, /* 15 */
        {1347.375714, -2372.21108226666, 1566.20719086287, -161.089864668723, 5.63065402793846}, /* 16 */
        {463.822363, -787.053152298914, 500.826971130479, -141.64095202131, 1.79148644473404} /* 17 */
    };
    // Answers
    double roots[][4] = {
        {0.08809808973990368, 0.1869183562478567}, /* 0 */
        {0.332355001781926, 0.4785910970082226},   /* 1 */
        {0.117153125076721, 0.3087847625899946},   /* 2 */
        {-0.61803398874989484820, 1.6180339887498948482, -6.5413812651491098445, -0.45861873485089015550}, /* 3 */
        {0.33995335944143, 0.55372069775034, 0.58805041057944, 0.87047717454653}, /* 4 */
        {0.06385398737126084, 0.267956069261246, 0.3712311334375205, 0.7818833436801197}, /* 5 */
        {0.4897764366689155, 0.6938173557314637, 0.7051071274833584, 0.9317275900380739}, /* 6 */
        {0.5399708576228983, 0.6990489875212664, 0.743593371226479, 0.8135827337080589}, /* 7 */
        {0.04537099624177689, 0.4012388080361285, 0.5227712126269068, 0.7295947383925314}, /* 8 */
        {0.2018746976609952, 0.55062936130567615948, 0.55062947295979712970, 0.8993841366044615}, /* 9 */
        {0.3734224150244674},   /* 10 */
        {0.4357365258470498, 0.4357365258470498}, /* 11 */
        {0.1991545128284868},                     /* 12 */
        {0.2023857558722718, 0.3160990356192021, 0.6699957312560749, 0.783709011003024}, /* 13 */
        {0.02374856039328096, 0.0925818978834563}, /* 14 */
        {},                                        /* 15 */
        {},                                        /* 16 */
        {0.01325662945878696, 0.8351858078659913}  /* 17 */
    };
    ...
    for (int i=0; i<nTests; ++i) {
        ...
        if (rc == nRoots[i]) {
            ...
            if (testArrayApprox(rs, xs, rc) == 0) {
                ...
                printf("\t%24.16lf, %24.16lf, %24.16lf, %24.16lf, %24.16lf \n", xs[0], xs[1], xs[2], xs[3], xs[4]);
            }
        } else {
            ++nMissed;
            ++nFailed;
